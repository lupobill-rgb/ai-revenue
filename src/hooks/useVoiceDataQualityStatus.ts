  ...
  const fetchStatus = useCallback(async () => {
    ...
    try {
      ...
      const { data: voiceSettingsData, error: voiceError } = await supabase
        ...
        .select("is_connected, elevenlabs_api_key, default_elevenlabs_voice_id, voice_provider")
      ...
      // - FALLBACK: check for key + voice id (migration resilience for older rows)
      const isExplicitlyConnected = voiceSettings?.is_connected === true;
      const hasElevenLabs = !!voiceSettings?.elevenlabs_api_key && !!voiceSettings?.default_elevenlabs_voice_id;
