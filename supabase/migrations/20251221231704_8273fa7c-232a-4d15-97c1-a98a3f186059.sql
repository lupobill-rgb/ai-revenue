-- Fix HS metrics RPC guard: service role calls may not populate request.jwt.claim.role.
-- Allow execution when current_user is service_role as well.
...
BEGIN
  ...
  -- HARD GUARD:
  -- - allow when DB role is service_role (service key)
  -- - allow when JWT role claim is service_role
  -- - allow authenticated platform admins
  IF v_db_role IS DISTINCT FROM 'service_role' AND v_role IS DISTINCT FROM 'service_role' THEN
