-- Create RPC function for weekly CFO snapshot using optimized query
...
BEGIN
  ...
  econ_action_rollup AS (
    SELECT
      ...
      -- Metrics where LOWER is better: payback_months, cac_blended
      COUNT(*) FILTER (
        WHERE ea.metric_id IN ('payback_months', 'cac_blended')
          AND ea.delta_direction = 'decrease'
      ) + 
      -- Metrics where HIGHER is better: margins, efficiency, revenue_per_fte
      COUNT(*) FILTER (
  ...
  cfo_gates AS (
    ...
    WHERE oc.invoked_at >= v_start_date
      ...
      AND jsonb_array_length(oc.cfo_gates_active) > 0
  ...
  SELECT
    ...
    COALESCE((t.config->>'cfo_expansion_enabled')::boolean, false) AS cfo_enabled,
