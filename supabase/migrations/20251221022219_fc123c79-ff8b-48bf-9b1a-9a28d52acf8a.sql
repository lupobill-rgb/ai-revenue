-- Recreate with explicit guard and lock search_path via ALTER
CREATE OR REPLACE FUNCTION public.get_horizontal_scaling_metrics(p_window_minutes int DEFAULT 5)
RETURNS jsonb
LANGUAGE plpgsql
STABLE
SECURITY DEFINER
...
REVOKE EXECUTE ON FUNCTION public.get_horizontal_scaling_metrics(int) FROM authenticated;
GRANT EXECUTE ON FUNCTION public.get_horizontal_scaling_metrics(int) TO service_role;
