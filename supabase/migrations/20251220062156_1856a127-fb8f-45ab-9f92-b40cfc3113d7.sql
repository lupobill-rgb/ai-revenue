-- Fix deploy_campaign so it does not mark campaigns as deployed/locked unless the campaign_run insert succeeds
...
BEGIN
  ...
  -- 1) Create campaign_run first (this is the operation most likely to fail due to permissions)
  BEGIN
